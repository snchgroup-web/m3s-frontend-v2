...
describe('InstitutionalProgramDesignGapCorrectionResult', () => {
  test('records both French corrections and the governed decision', () => {
    ...
    expect(screen.getByText(/Je confirme PGM-CON-COR-002 V0.1/i)).toBeInTheDocument();
  ...
  test('preserves the English operational boundary', () => {
    ...
    expect(screen.getByText('Two corrections executed, zero evidence accepted')).toBeInTheDocument();
  ...
  test('renders the German result gate', () => {
    ...
    expect(screen.getByText('Zwei Korrekturen ausgeführt, null Nachweise angenommen')).toBeInTheDocument();
    expect(screen.getByText(/Nächste gebündelte Bestätigung/i)).toBeInTheDocument();
    expect(screen.getByText(/PGM-CON-COR-002 V0.1 als Ausführungsprotokoll/i)).toBeInTheDocument();
  });
});
