...
const COPY = {
  FR: {
    eyebrow: 'DÉSIGNATION DES CATÉGORIES · CANDIDAT V0.1 · 28-08-2026',
    title: 'Désigner les catégories réellement utilisées par REF-01',
    intro: 'Le cadre LEGAL V1.0 est validé en interne. Cette fiche suivante rattache les catégories observées à leur fonction responsable sans copier de pièce personnelle, RH ou financière dans REF-01.',
    ...
    labels: { category: 'Catégorie', designation: 'Désignation candidate', owner: 'Fonction responsable', trigger: 'Contrôle avant application' },
    ...
    status: 'CADRE LEGAL VALIDÉ · Désignation candidate préparée. Zéro règle automatisée, suppression, archive ou opération GED.',
    next: 'Prochain arbitrage humain : confirmer ou amender REF-01-G1-AUT-02-03-004 V0.1, puis attribuer responsables et déclencheurs au niveau des registres autorisés.',
  ...
  EN: {
    eyebrow: 'CATEGORY DESIGNATION · V0.1 CANDIDATE · 28 AUG 2026',
    title: 'Designate the categories actually used by REF-01',
    intro: 'The V1.0 LEGAL framework is internally validated. This next file maps observed categories to their responsible function without copying personal, HR or financial records into REF-01.',
    ...
    labels: { category: 'Category', designation: 'Candidate designation', owner: 'Responsible function', trigger: 'Control before application' },
    ...
    status: 'LEGAL FRAMEWORK VALIDATED · Candidate designation prepared. Zero automated rule, deletion, archive or DMS operation.',
    next: 'Next human decision: confirm or amend REF-01-G1-AUT-02-03-004 V0.1, then assign owners and triggers within authorised registers.',
  ...
  DE: {
    eyebrow: 'KATEGORIENZUORDNUNG · KANDIDAT V0.1 · 28.08.2026',
    title: 'Die von REF-01 tatsächlich genutzten Kategorien bestimmen',
    intro: 'Der LEGAL-Rahmen V1.0 ist intern validiert. Diese nächste Akte ordnet beobachtete Kategorien ihrer verantwortlichen Funktion zu, ohne persönliche, Personal- oder Finanzunterlagen in REF-01 zu kopieren.',
    ...
    labels: { category: 'Kategorie', designation: 'Kandidatenzuordnung', owner: 'Verantwortliche Funktion', trigger: 'Kontrolle vor Anwendung' },
    ...
    status: 'LEGAL-RAHMEN VALIDIERT · Kandidatenzuordnung vorbereitet. Null automatisierte Regel, Löschung, Archivierung oder DMS-Operation.',
    next: 'Nächster menschlicher Entscheid: REF-01-G1-AUT-02-03-004 V0.1 bestätigen oder ändern und danach Verantwortliche und Auslöser in autorisierten Registern zuweisen.',
...
const InstitutionalPeopleTeamsGateG1RetentionCategoryDesignation = ({ language = 'FR' }) => {
  ...
  return (
    <section data-testid="ref01-g1-retention-category-designation" className="mt-4 rounded-md border border-emerald-800/70 bg-emerald-950/10 p-3 sm:p-4" aria-labelledby="ref01-g1-retention-category-designation-title">
      ...
      <article className="mt-4 m3s-raised p-3 sm:p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div className="flex items-start gap-2"><Link2 className="mt-0.5 shrink-0 text-sky-300" size={18} aria-hidden="true" /><div><h6 className="text-sm font-semibold text-slate-100">REF-01-G1-AUT-02-03-004 · V0.1</h6><p className="mt-1 text-xs font-semibold text-sky-200">{t.title}</p></div></div><span className="rounded-md border border-amber-700/70 bg-amber-950/25 px-2 py-1 text-[10px] font-semibold text-amber-100">CANDIDAT · À CONFIRMER</span></div>
