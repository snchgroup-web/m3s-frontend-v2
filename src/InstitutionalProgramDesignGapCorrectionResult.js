...
const COPY = {
  FR: {
    eyebrow: 'RELEVÉ D’EXÉCUTION CONFIRMÉ · PGM-CON-COR-002 · V1.0 · 02-09-2026',
    title: 'Deux corrections confirmées, aucune preuve acceptée',
    intro: 'PGM-DEC-011 consigne le GO d’exécution et PGM-DEC-012 confirme le relevé. Les résultats techniques restent distincts de toute acceptation probatoire.',
    ...
    confirmationRecord: { id: 'PGM-DEC-012', version: 'V1.0', status: 'Relevé d’exécution confirmé', author: 'Cheikh Ndiaye', date: '02-09-2026', decision: 'PGM-CON-COR-002 V0.1 est confirmé et promu en V1.0 comme relevé d’exécution de COR-01 et COR-02.', evidence: 'Confirmation explicite de Cheikh dans la session du 02-09-2026 : « Je confirme PGM-CON-COR-002 V0.1 comme relevé d’exécution de COR-01 et COR-02, sans accepter de preuve ni ouvrir CON-01, CON-05, REF-02 ou L2. »', limit: 'La confirmation porte sur le relevé d’exécution uniquement. Elle n’accepte aucune preuve et ne rouvre ni CON-01, CON-05, REF-02 ni L2.' },
    next: 'Contrôle post-correction candidat',
    confirmation: 'Les deux résultats sont repris dans PGM-CON-REV-003 V0.1 pour un contrôle post-correction séparé, sans acceptation probatoire.',
    boundary: 'Résultat confirmé : 0 preuve acceptée, 0 progression calculée et 0 périmètre institutionnel rouvert.'
  },
  EN: {
    eyebrow: 'CONFIRMED EXECUTION RECORD · PGM-CON-COR-002 · V1.0 · 2 SEP 2026',
    title: 'Two corrections confirmed, zero evidence accepted',
    intro: 'PGM-DEC-011 records the execution GO and PGM-DEC-012 confirms the record. Technical results remain separate from evidentiary acceptance.',
    ...
    confirmationRecord: { id: 'PGM-DEC-012', version: 'V1.0', status: 'Execution record confirmed', author: 'Cheikh Ndiaye', date: '2 Sep 2026', decision: 'PGM-CON-COR-002 V0.1 is confirmed and promoted to V1.0 as the execution record for COR-01 and COR-02.', evidence: 'Cheikh’s explicit confirmation in the 2 Sep 2026 session, retained in French: “Je confirme PGM-CON-COR-002 V0.1 comme relevé d’exécution de COR-01 et COR-02, sans accepter de preuve ni ouvrir CON-01, CON-05, REF-02 ou L2.”', limit: 'Confirmation covers the execution record only. It accepts no evidence and does not reopen CON-01, CON-05, REF-02 or L2.' },
    next: 'Candidate post-correction review',
    confirmation: 'Both results are carried into PGM-CON-REV-003 V0.1 for a separate post-correction review, without evidentiary acceptance.',
    boundary: 'Confirmed result: zero evidence accepted, zero calculated progress and zero institutional scope reopened.'
  },
  DE: {
    eyebrow: 'BESTÄTIGTES AUSFÜHRUNGSPROTOKOLL · PGM-CON-COR-002 · V1.0 · 02.09.2026',
    title: 'Zwei Korrekturen bestätigt, null Nachweise angenommen',
    intro: 'PGM-DEC-011 erfasst das Ausführungs-GO und PGM-DEC-012 bestätigt das Protokoll. Technische Ergebnisse bleiben von jeder Nachweisannahme getrennt.',
    ...
    confirmationRecord: { id: 'PGM-DEC-012', version: 'V1.0', status: 'Ausführungsprotokoll bestätigt', author: 'Cheikh Ndiaye', date: '02.09.2026', decision: 'PGM-CON-COR-002 V0.1 wird als Ausführungsprotokoll für COR-01 und COR-02 bestätigt und zu V1.0 befördert.', evidence: 'Ausdrückliche Bestätigung von Cheikh in der Sitzung vom 02.09.2026, im französischen Originalwortlaut: „Je confirme PGM-CON-COR-002 V0.1 comme relevé d’exécution de COR-01 et COR-02, sans accepter de preuve ni ouvrir CON-01, CON-05, REF-02 ou L2.“', limit: 'Die Bestätigung betrifft nur das Ausführungsprotokoll. Sie nimmt keine Nachweise an und öffnet weder CON-01, CON-05, REF-02 noch L2 erneut.' },
    next: 'Kandidat für Nachkorrekturprüfung',
    confirmation: 'Beide Ergebnisse werden in PGM-CON-REV-003 V0.1 für eine getrennte Nachkorrekturprüfung übernommen, ohne Nachweisannahme.',
    boundary: 'Bestätigtes Ergebnis: null Nachweise angenommen, null Fortschritt berechnet und kein institutioneller Umfang wieder geöffnet.'
  }
};
...
const InstitutionalProgramDesignGapCorrectionResult = ({ language = 'FR' }) => {
  ...
  return (
    <section id="institutional-program-design-gap-correction-result" data-testid="institutional-program-design-gap-correction-result" className="scroll-mt-24 m3s-panel p-4 sm:p-5">
      ...
      <GovernedDecisionRecord labels={t.recordLabels} record={t.confirmationRecord} />
