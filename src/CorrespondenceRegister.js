...
import { ADMINISTRATION_CORRESPONDENCE_WRITE_PERMISSION, hasPermission } from './accessControl';
...
const CorrespondenceRegister = ({ language = 'FR' }) => {
  ...
  const prepareCv = () => { if (!canWrite) return; const base = defaultForm(); setEditing('new'); setForm({ ...base, channelIndex: 0, subject: t.cvSubject, categoryIndex: 0, confidentialityIndex: 2, statusIndex: 1, next: t.cvNext }); setMessage(''); };
  ...
  return (
    <div className="mt-8 border-t border-slate-700 pt-6">
      ...
      <div className="mt-4 overflow-x-auto rounded-lg border border-slate-700"><table className="min-w-[980px] w-full text-left text-sm"><thead className="bg-slate-900/70 text-xs uppercase text-slate-300"><tr><th className="px-4 py-3">ID</th><th className="px-4 py-3">{t.fields.date}</th><th className="px-4 py-3">{t.fields.subject}</th><th className="px-4 py-3">{t.fields.channel}</th><th className="px-4 py-3">{t.fields.owner}</th><th className="px-4 py-3">{t.fields.status}</th>{canWrite && <th className="px-4 py-3">Actions</th>}</tr></thead><tbody>{visible.map(item => <tr key={item.id} className={`border-t border-slate-700 text-slate-300 transition ${canWrite ? 'cursor-pointer hover:bg-blue-950/35' : ''}`} tabIndex={canWrite ? 0 : undefined} onClick={canWrite ? () => open(item) : undefined} onKeyDown={canWrite ? event => { if (event.key === 'Enter') open(item); } : undefined}><td className="px-4 py-3 font-mono text-xs text-cyan-200">{item.id}</td><td className="px-4 py-3">{item.date}</td><td className="px-4 py-3 font-semibold text-slate-100">{item.subject}</td><td className="px-4 py-3">{t.channels[item.channelIndex]}</td><td className="px-4 py-3">{item.owner}</td><td className="px-4 py-3">{t.statuses[item.statusIndex]}</td>{canWrite && <td className="px-4 py-3"><div className="flex gap-2"><button type="button" className="m3s-icon-button text-blue-300" title={t.edit} onClick={event => { event.stopPropagation(); open(item); }} disabled={busy}><Edit2 size={16} /></button><button type="button" className="m3s-icon-button text-red-300" title={t.delete} onClick={event => { event.stopPropagation(); remove(item); }} disabled={busy}><Trash2 size={16} /></button></div></td>}</tr>)}</tbody></table>{!visible.length && <p className="p-8 text-center text-sm text-slate-400">{t.empty}</p>}</div>
      {canWrite && editing && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 p-4" role="presentation"><form onSubmit={save} className="m3s-panel max-h-[92vh] w-full max-w-4xl overflow-y-auto p-5 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="correspondence-form-title"><div className="flex items-start justify-between gap-3"><h2 id="correspondence-form-title" className="m3s-page-title">{editing === 'new' ? t.add : t.edit}</h2><button type="button" className="m3s-icon-button" onClick={close} aria-label={t.close}><X size={20} /></button></div><div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
