...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.8 · 26-08-2026')).toBeInTheDocument();
  ...
  expect(within(architectureDecision).getByText('REF-01-ADR-001 · V0.1 · 26-08-2026', { exact: false })).toBeInTheDocument();
  ...
  expect(within(architectureDecision).getByText('CANDIDATE · A CONFIRMER')).toBeInTheDocument();
  expect(control.getByText(/confirmer, corriger ou rejeter REF-01-ADR-001 V0.1/)).toBeInTheDocument();
  expect(control.getByText(/décisions sur le lot : 8/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByText(/confirm, amend or reject REF-01-ADR-001 V0.1/)).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-003 · V1.0' })).toBeInTheDocument();
  expect(screen.getByText(/REF-01-ADR-001 V0.1 und die Verantwortungsmatrix/)).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getAllByText('Governed decision record')).toHaveLength(18);
  ...
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(18);
