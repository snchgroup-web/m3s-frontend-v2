...
test('opens the current REF-01 technical-wave reassessment in a lightweight dedicated programme view', () => {
  renderDashboardNavigation({}, '/?view=program&focus=ref01-aut0205');

  expect(screen.getByRole('heading', { name: 'Outbox cadrée · Vague technique à réévaluer' })).toBeInTheDocument();
  expect(screen.getByRole('link', { name: 'Ouvrir le programme complet' })).toHaveAttribute('href', '/?view=program&returnTo=ref01-aut0205#institutional-ref01-g1-aut-02-05-confirmation');
...
test('offers an explicit return to the lightweight AUT-02-05 decision from the full programme', () => {
  renderDashboardNavigation({}, '/?view=program&returnTo=ref01-aut0205');

  expect(screen.getByRole('heading', { name: 'De l’idée à une institution durable' })).toBeInTheDocument();
  expect(screen.getByRole('link', { name: 'Retour à l’arbitrage AUT-02-05' })).toHaveAttribute('href', '/?view=program&focus=ref01-aut0205');
});
...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.67 · 30-08-2026')).toBeInTheDocument();
  ...
  const elevenFieldMatrices = screen.getByRole('heading', { name: 'Conserver les chaînes confirmées et réévaluer la vague sans ouvrir de droit' }).closest('section');
  ...
  expect(within(waveCandidate).getByText('Tests lancés').closest('article')).toHaveTextContent('0');
  expect(control.getByText(/décisions sur le lot : 61/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Keep the confirmed chains and reassess the wave without opening a right' })).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'Bestätigte Dokumentketten bewahren und die Welle ohne Rechteöffnung neu bewerten' })).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governed decision record')).toHaveLength(71);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(71);
