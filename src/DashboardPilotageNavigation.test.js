...
test('shows the governed institutional programme without inventing progress', () => {
  ...
  expect(screen.getByText('Matrice de cadrage V0.1 · 29 composantes')).toBeInTheDocument();
  ...
  expect(screen.getByTestId('institutional-program-design-gap-correction-result')).toBeInTheDocument();
  expect(screen.getAllByTestId('institutional-program-design-gap-result')).toHaveLength(2);
  expect(screen.getByTestId('institutional-program-design-post-correction-review')).toBeInTheDocument();
  expect(screen.getAllByTestId('institutional-program-design-post-correction-control')).toHaveLength(2);
...
test('opens the authorised bounded collection and keeps operational boundaries in the lightweight Fast Track view', () => {
  ...
  expect(screen.getByRole('heading', { name: 'G1 en attente gouvernée · trajectoire globale reprise' })).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governed decision record')).toHaveLength(98);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(98);
