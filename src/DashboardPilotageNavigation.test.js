...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.58 · 29-08-2026')).toBeInTheDocument();
  ...
  const elevenFieldMatrices = screen.getByRole('heading', { name: 'Confirmer le registre et préparer ses portes sans ouvrir de droit' }).closest('section');
  ...
  expect(within(accessOpeningGates).getByRole('heading', { name: 'REF-01-G1-AUT-02-02-007 · V0.1' })).toBeInTheDocument();
  ...
  expect(control.getByText(/décisions sur le lot : 52/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Confirm the register and prepare its gates without opening a right' })).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-G1-AUT-02-02-007 · V0.1' })).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'Das Register bestätigen und seine Tore ohne Rechteöffnung vorbereiten' })).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governed decision record')).toHaveLength(62);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(62);
