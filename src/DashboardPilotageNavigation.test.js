...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.16 · 26-08-2026')).toBeInTheDocument();
  ...
  const recipientCandidate = screen.getByRole('heading', { name: 'Qualifier les destinataires sans inscrire de nom réel' }).closest('section');
  ...
  expect(within(recipientCandidate).getAllByText('PROFIL CANDIDAT · AUCUN NOM')).toHaveLength(4);
  expect(control.getByText(/confirmer ou amender REF-01-G1-REC-001 V0.1/)).toBeInTheDocument();
  expect(control.getByText(/décisions sur le lot : 13/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Qualify recipients without recording a real name' })).toBeInTheDocument();
  expect(screen.getAllByText('CANDIDATE PROFILE · NO NAME')).toHaveLength(4);
  ...
  expect(screen.getByText(/confirm or amend REF-01-G1-REC-001 V0.1/)).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-016 · V1.0' })).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: 'Empfänger qualifizieren, ohne einen realen Namen zu erfassen' })).toBeInTheDocument();
  expect(screen.getAllByText('KANDIDATENPROFIL · KEIN NAME')).toHaveLength(4);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-003 · V1.0' })).toBeInTheDocument();
  expect(screen.getByText(/REF-01-G1-REC-001 V0.1 bestätigen oder ändern/)).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getAllByText('Governed decision record')).toHaveLength(23);
  ...
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(23);
