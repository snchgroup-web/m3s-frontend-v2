...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.14 · 26-08-2026')).toBeInTheDocument();
  ...
  const collectionPack = screen.getByRole('heading', { name: 'Préparer la collecte contrôlée sans démarrer la consultation' }).closest('section');
  ...
  expect(within(collectionPack).getByText('PAQUET PRET POUR ARBITRAGE · Sept emplacements vides, zéro collecte et zéro destinataire autorisé.')).toBeInTheDocument();
  expect(control.getByText(/confirmer ou amender REF-01-G1-COL-001 V0.1/)).toBeInTheDocument();
  expect(control.getByText(/décisions sur le lot : 11/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Prepare controlled collection without starting consultation' })).toBeInTheDocument();
  expect(screen.getByText('PACKAGE READY FOR DECISION · Seven empty slots, zero collection and zero authorised recipients.')).toBeInTheDocument();
  ...
  expect(screen.getByText(/confirm or amend REF-01-G1-COL-001 V0.1/)).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'Kontrollierte Sammlung vorbereiten, ohne die Konsultation zu starten' })).toBeInTheDocument();
  expect(screen.getByText('PAKET ENTSCHEIDUNGSBEREIT · Sieben leere Stellen, null Sammlung und null autorisierte Empfänger.')).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-003 · V1.0' })).toBeInTheDocument();
  expect(screen.getByText(/REF-01-G1-COL-001 V0.1, Umfang, Kanäle und Verantwortung/)).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getAllByText('Governed decision record')).toHaveLength(21);
  ...
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(21);
