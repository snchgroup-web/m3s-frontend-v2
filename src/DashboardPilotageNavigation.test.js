...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.15 · 26-08-2026')).toBeInTheDocument();
  ...
  const requestCandidate = screen.getByRole('heading', { name: 'Préparer les demandes sans nommer ni contacter un destinataire' }).closest('section');
  ...
  expect(within(requestCandidate).getAllByText('BROUILLON NON ENVOYABLE')).toHaveLength(4);
  expect(control.getByText(/confirmer ou amender REF-01-G1-REQ-001 V0.1/)).toBeInTheDocument();
  expect(control.getByText(/décisions sur le lot : 12/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Prepare requests without naming or contacting a recipient' })).toBeInTheDocument();
  expect(screen.getAllByText('UNSENDABLE DRAFT')).toHaveLength(4);
  ...
  expect(screen.getByText(/confirm or amend REF-01-G1-REQ-001 V0.1/)).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'Anfragen vorbereiten, ohne Empfänger zu benennen oder zu kontaktieren' })).toBeInTheDocument();
  expect(screen.getAllByText('NICHT VERSENDBARER ENTWURF')).toHaveLength(4);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-003 · V1.0' })).toBeInTheDocument();
  expect(screen.getByText(/REF-01-G1-REQ-001 V0.1 bestätigen oder ändern/)).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getAllByText('Governed decision record')).toHaveLength(22);
  ...
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(22);
