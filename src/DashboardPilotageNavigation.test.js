...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.44 · 28-08-2026')).toBeInTheDocument();
  ...
  const elevenFieldMatrices = screen.getByRole('heading', { name: 'Confirmer les valeurs et instruire les deux champs LEGAL' }).closest('section');
  ...
  expect(within(elevenFieldMatrices).getAllByText('SOURCÉ · CONFIRMÉ')).toHaveLength(4);
  expect(within(elevenFieldMatrices).getAllByText('TRAVAIL · CONFIRMÉ')).toHaveLength(5);
  expect(within(elevenFieldMatrices).getAllByText('LEGAL SOURCÉ · À VALIDER')).toHaveLength(2);
  expect(within(elevenFieldMatrices).getByText('Valeurs confirmées').closest('article')).toHaveTextContent('9/9');
  expect(within(elevenFieldMatrices).getByText('Champs LEGAL instruits').closest('article')).toHaveTextContent('2/2');
  ...
  expect(control.getByText(/décisions sur le lot : 39/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Confirm the values and establish the two LEGAL fields' })).toBeInTheDocument();
  ...
  expect(screen.getAllByText('LEGAL SOURCED · TO VALIDATE')).toHaveLength(2);
  ...
  expect(screen.getByRole('heading', { name: 'Werte bestätigen und die beiden LEGAL-Felder bestimmen' })).toBeInTheDocument();
  ...
  expect(screen.getAllByText('LEGAL BELEGT · ZU PRÜFEN')).toHaveLength(2);
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-041 · V1.0' })).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-042 · V1.0' })).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governed decision record')).toHaveLength(49);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(49);
