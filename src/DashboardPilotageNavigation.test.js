...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.43 · 28-08-2026')).toBeInTheDocument();
  ...
  const elevenFieldMatrices = screen.getByRole('heading', { name: 'Confirmer les structures et renseigner ce que les preuves permettent' }).closest('section');
  ...
  expect(within(elevenFieldMatrices).getAllByText('SOURCÉ · DÉJÀ OBSERVÉ')).toHaveLength(4);
  expect(within(elevenFieldMatrices).getAllByText('PARTIEL · À ARBITRER')).toHaveLength(5);
  expect(within(elevenFieldMatrices).getAllByText('OUVERT · SOURCE REQUISE')).toHaveLength(2);
  expect(within(elevenFieldMatrices).getByText('Champs qualifiés').closest('article')).toHaveTextContent('11/11');
  expect(within(elevenFieldMatrices).getByText('Valeurs sourcées').closest('article')).toHaveTextContent('4');
  ...
  expect(within(elevenFieldMatrices).getByText(/Aucune durée applicable n’est fixée/)).toBeInTheDocument();
  expect(within(elevenFieldMatrices).getByText(/Lecture seule de l’annuaire C2 assaini/)).toBeInTheDocument();
  expect(control.getByText(/décisions sur le lot : 38/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Confirm the structures and provide what the evidence supports' })).toBeInTheDocument();
  expect(screen.getAllByText('SOURCED · ALREADY OBSERVED')).toHaveLength(4);
  expect(screen.getAllByText('PARTIAL · DECISION REQUIRED')).toHaveLength(5);
  expect(screen.getAllByText('OPEN · SOURCE REQUIRED')).toHaveLength(2);
  ...
  expect(screen.getByRole('heading', { name: 'Strukturen bestätigen und belegen, was die Nachweise erlauben' })).toBeInTheDocument();
  expect(screen.getAllByText('BELEGT · BEREITS BEOBACHTET')).toHaveLength(4);
  expect(screen.getAllByText('TEILWEISE · ENTSCHEID NÖTIG')).toHaveLength(5);
  expect(screen.getAllByText('OFFEN · QUELLE NÖTIG')).toHaveLength(2);
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-041 · V1.0' })).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governed decision record')).toHaveLength(48);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(48);
