...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.17 · 26-08-2026')).toBeInTheDocument();
  ...
  const namedRecipientCandidate = screen.getByRole('heading', { name: 'Préparer les emplacements sans enregistrer d’identité' }).closest('section');
  ...
  expect(within(namedRecipientCandidate).getAllByText('VIDE · NON AUTORISÉ')).toHaveLength(4);
  expect(control.getByText(/confirmer ou amender REF-01-G1-NAM-001 V0.1/)).toBeInTheDocument();
  expect(control.getByText(/décisions sur le lot : 14/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Prepare slots without recording an identity' })).toBeInTheDocument();
  expect(screen.getAllByText('EMPTY · NOT AUTHORISED')).toHaveLength(4);
  ...
  expect(screen.getByText(/confirm or amend REF-01-G1-NAM-001 V0.1/)).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-017 · V1.0' })).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: 'Leere Stellen vorbereiten, ohne eine Identität zu erfassen' })).toBeInTheDocument();
  expect(screen.getAllByText('LEER · NICHT AUTORISIERT')).toHaveLength(4);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-003 · V1.0' })).toBeInTheDocument();
  expect(screen.getByText(/REF-01-G1-NAM-001 V0.1 bestätigen oder ändern/)).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getAllByText('Governed decision record')).toHaveLength(24);
  ...
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(24);
