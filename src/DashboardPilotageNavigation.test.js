...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.59 · 29-08-2026')).toBeInTheDocument();
  ...
  const elevenFieldMatrices = screen.getByRole('heading', { name: 'Confirmer les portes et préparer le verdict sans ouvrir de droit' }).closest('section');
  ...
  expect(within(accessOpeningDecision).getByRole('heading', { name: 'REF-01-G1-AUT-02-02-008 · V0.1' })).toBeInTheDocument();
  ...
  expect(control.getByText(/décisions sur le lot : 53/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Confirm the gates and prepare the outcome without opening a right' })).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-G1-AUT-02-02-008 · V0.1' })).toBeInTheDocument();
  ...
  expect(within(screen.getByTestId('ref01-g1-least-privilege-access-opening-decision-sheet')).getByText('Four proposed completion rules')).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'Die Tore bestätigen und das Ergebnis ohne Rechteöffnung vorbereiten' })).toBeInTheDocument();
  ...
  expect(within(screen.getByTestId('ref01-g1-least-privilege-access-opening-decision-sheet')).getByText('Vier vorgeschlagene Vollständigkeitsregeln')).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governed decision record')).toHaveLength(63);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(63);
