...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.18 · 26-08-2026')).toBeInTheDocument();
  ...
  const namedAuthorisationCandidate = screen.getByRole('heading', { name: 'Autoriser chaque future inscription séparément' }).closest('section');
  ...
  expect(within(namedAuthorisationCandidate).getAllByText('DOSSIER VIDE · NON AUTORISÉ')).toHaveLength(4);
  expect(control.getByText(/confirmer ou amender REF-01-G1-AUT-001 V0.1/)).toBeInTheDocument();
  expect(control.getByText(/décisions sur le lot : 15/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Authorise each future entry separately' })).toBeInTheDocument();
  expect(screen.getAllByText('EMPTY FILE · NOT AUTHORISED')).toHaveLength(4);
  ...
  expect(screen.getByText(/confirm or amend REF-01-G1-AUT-001 V0.1/)).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-018 · V1.0' })).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: 'Jeden künftigen Eintrag getrennt autorisieren' })).toBeInTheDocument();
  expect(screen.getAllByText('LEERES DOSSIER · NICHT AUTORISIERT')).toHaveLength(4);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-003 · V1.0' })).toBeInTheDocument();
  expect(screen.getByText(/REF-01-G1-AUT-001 V0.1 bestätigen oder ändern/)).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getAllByText('Governed decision record')).toHaveLength(25);
  ...
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(25);
