...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.56 · 29-08-2026')).toBeInTheDocument();
  ...
  const elevenFieldMatrices = screen.getByRole('heading', { name: 'Confirmer les tests d’accès et préparer l’attribution sans ouvrir de droit' }).closest('section');
  ...
  expect(within(grantWithdrawalProtocol).getByRole('heading', { name: 'REF-01-G1-AUT-02-02-005 · V0.1' })).toBeInTheDocument();
  ...
  expect(control.getByText(/décisions sur le lot : 50/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Confirm access tests and prepare grants without opening a right' })).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-G1-AUT-02-02-005 · V0.1' })).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'Zugriffstests bestätigen und Zuweisungen ohne Rechteöffnung vorbereiten' })).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governed decision record')).toHaveLength(60);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(60);
