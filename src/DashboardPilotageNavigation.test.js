...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.45 · 28-08-2026')).toBeInTheDocument();
  ...
  const elevenFieldMatrices = screen.getByRole('heading', { name: 'Valider les deux champs LEGAL et désigner les catégories' }).closest('section');
  ...
  expect(within(categoryDesignation).getByRole('heading', { name: 'REF-01-G1-AUT-02-03-004 · V0.1' })).toBeInTheDocument();
  ...
  expect(control.getByText(/décisions sur le lot : 40/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Validate the two LEGAL fields and designate categories' })).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-G1-AUT-02-03-004 · V0.1' })).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'Die zwei LEGAL-Felder validieren und Kategorien bestimmen' })).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governed decision record')).toHaveLength(50);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-040 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(50);
