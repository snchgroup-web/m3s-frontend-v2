...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.31 · 27-08-2026')).toBeInTheDocument();
  ...
  const priorityOnePackages = screen.getByRole('heading', { name: 'Préparer les deux lots de priorité 1 sans ouvrir d’accès ni toucher aux pièces' }).closest('section');
  expect(within(priorityOnePackages).getAllByTestId('ref01-g1-priority-one-package')).toHaveLength(2);
  expect(within(priorityOnePackages).getByText(/confirmer ou amender REF-01-G1-PKG-01-001 V0.1 et REF-01-G1-PKG-02-001 V0.1/)).toBeInTheDocument();
  expect(control.getByText(/décisions sur le lot : 26/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Prepare both priority-one packages without opening access or changing records' })).toBeInTheDocument();
  ...
  expect(screen.getAllByText(/One grouped decision is required/).length).toBeGreaterThan(0);
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-029 · V1.0' })).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: 'Beide Priorität-1-Pakete vorbereiten, ohne Zugriff zu öffnen oder Unterlagen zu ändern' })).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-003 · V1.0' })).toBeInTheDocument();
  expect(screen.getAllByText(/Ein gebündelter Entscheid ist erforderlich/).length).toBeGreaterThan(0);
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getAllByText('Governed decision record')).toHaveLength(36);
  ...
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(36);
