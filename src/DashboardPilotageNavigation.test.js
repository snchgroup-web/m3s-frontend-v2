...
test('frames REF-01 people and teams without exposing RH-001 records or promoting a master source', () => {
  ...
  expect(control.getByText('CONTROLE DETAILLE 1/11 · REF-01 · V1.7 · 26-08-2026')).toBeInTheDocument();
  ...
  expect(within(implementationProposal).getByText('RECOMMANDATION · A VALIDER')).toBeInTheDocument();
  ...
  expect(control.getByText(/confirmer, corriger ou rejeter REF-01-IMP-001 V0.1 avant toute implémentation/)).toBeInTheDocument();
  expect(control.getByText(/décisions sur le lot : 7/)).toBeInTheDocument();
...
test('translates the REF-01 people and teams control in English and German', () => {
  ...
  expect(screen.getByText(/confirm, amend or reject REF-01-IMP-001 V0.1 before any implementation/)).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: 'REF-01-DEC-003 · V1.0' })).toBeInTheDocument();
  expect(screen.getByText(/REF-01-IMP-001 V0.1 vor jeder Umsetzung bestätigen/)).toBeInTheDocument();
...
test('translates the CNS decision matrix in English and German', () => {
  ...
  expect(screen.getAllByText('Governed decision record')).toHaveLength(17);
  ...
  expect(screen.getAllByText('Governance-konformer Entscheidnachweis')).toHaveLength(17);
